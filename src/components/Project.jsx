...
export default function Project({project,addTask,index}){
    ...
    return(
        <main className="w-full px-10 py-20 mr-56">
            ...
            <div className="flex flex-col gap-4 mt-4">
                ...
                <div className="h-11">
                    <input onChange={(event)=>setName(event.target.value)} className="border-4 bg-stone-200 w-80 h-full  p-3 focus:border-blue-800 rounded-md"/>
                    <button onClick={()=>addTask(name,index)} className="ml-5 text-stone-600 text-2xl">Add Task</button>
                </div>
                ...
                {project.tasks.length>0 && 
                    project.tasks.map((task,index)=>(
                        <div key={index} className="flex justify-between">
                            ...
                            <button className="text-xl hover:text-red-700 text-black">Clear</button>
                        </div>
                    ))
                }
                </div>
